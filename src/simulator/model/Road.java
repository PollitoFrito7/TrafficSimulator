...
public abstract class Road extends SimulatedObject {
	...
	Road(String id, Junction srcJunct, Junction destJunct, int maxSpeed, int contLimit, int length, Weather weather) {
		super(id);
		if (srcJunct == null || destJunct == null)
			throw new IllegalArgumentException("The junctions can't be NULL");
		_srcJunct = srcJunct;
		...
		if (length <= 0)
			throw new IllegalArgumentException("The length of the road must be positive.");
		_length = length;

		if (maxSpeed <= 0)
			throw new IllegalArgumentException("The maximum speed of the road must be positive.");
		_maxSpeed = maxSpeed;

		_curMaxSpeed = maxSpeed;

		if (contLimit <= 0)
			throw new IllegalArgumentException("The maximum contamination must be positive");
	...
	protected void addContamination(int c) {
		if (c < 0)
			throw new IllegalArgumentException("The contamination must be positive");
		else
	...
	public JSONObject report() {
		...
		road.put("weather", _weather.toString());
